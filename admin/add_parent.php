...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        $stmt = $pdo->prepare("INSERT INTO users (full_name, username, password, role) VALUES (?, ?, ?, 'parent')");
        if ($stmt->execute([$full_name, $username, $password])) {
            $success_msg = "Parent account created successfully!";
        } else {
            $error_msg = "Failed to create parent account.";
        }
